...
TEST(BufferManager, WriteBufferedPage) {
	...
	// Write to page
	{
		auto &page = buffer_manager.fix_page(348, 1, true, nullptr, false);
	...
	// Read from page
	{
		...
		auto &page = buffer_manager.fix_page(348, 1, false, nullptr, false);
...
TEST(BufferManager, EvictPage) {
	...
	auto &page2 = buffer_manager.fix_page(169, 2, true, nullptr, false);
...
TEST(BufferManager, PersistentEviction) {
	...
	bbbtree::BufferManager buffer_manager{page_size, 1};
	{
		auto &frame = buffer_manager.fix_page(348, 1, true, nullptr, false);
	...
	// Evict page
	{
		auto &frame = buffer_manager.fix_page(169, 1, true, nullptr, false);
...
TEST(BufferManager, BufferFull) {
	...
	EXPECT_THROW(buffer_manager.fix_page(169, 2, true, nullptr, false),
