...
namespace
{
    ...
    protected:
        void RestructFSI()
        {
            fsi = std::make_unique<bbbtree::FSISegment>(FSI_SEGMENT_ID, *buffer_manager);
        }
        ...
        std::unique_ptr<bbbtree::BufferManager> buffer_manager = std::make_unique<bbbtree::BufferManager>(1024, 10);
        std::unique_ptr<bbbtree::FSISegment> fsi = std::make_unique<bbbtree::FSISegment>(FSI_SEGMENT_ID, *buffer_manager);
        std::unique_ptr<bbbtree::SPSegment> sp_segment = std::make_unique<bbbtree::SPSegment>(SP_SEGMENT_ID, *buffer_manager, *fsi);
    };
