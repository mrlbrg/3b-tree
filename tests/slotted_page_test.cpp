...
TEST(SlottedPage, Constructor)
{
    ...
    for (auto *data = page->get_data() + sizeof(bbbtree::SlottedPage); data < page->get_data() + page_size; ++data)
    {
...
TEST(SlottedPage, PageFull)
{
    ...
    // Allocating space exceeding page space should throw and not allocate anything.
...
TEST(SlottedPage, Allocate)
{
    ...
    EXPECT_NO_THROW(page->allocate(page_size - sizeof(bbbtree::SlottedPage) - sizeof(bbbtree::SlottedPage::Slot) * 2 - 1, page_size));
    EXPECT_EQ(page->header.slot_count, 2);
    EXPECT_EQ(page->header.data_start, sizeof(bbbtree::SlottedPage) + sizeof(bbbtree::SlottedPage::Slot) * 2);
