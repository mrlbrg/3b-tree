...
namespace bbbtree
{
    ...
    // -----------------------------------------------------------------
    template <LessEqualComparable KeyT, typename ValueT>
    BTree<KeyT, ValueT>::InnerNode::InnerNode(uint32_t page_size, uint16_t level, PageID first_child, PageID upper) : Node(page_size, level, 1), upper(upper)
    {
    ...
    PageID BTree<KeyT, ValueT>::InnerNode::lower_bound(const KeyT &key)
    {
        ...
        auto comp = [&](const Slot &slot, const KeyT &key) -> bool
        {
            ...
            // Get key from slots
            const auto &slot_key = *reinterpret_cast<KeyT *>(this->get_data() + slot.offset);
    ...
    // -----------------------------------------------------------------
    template <LessEqualComparable KeyT, typename ValueT>
    BTree<KeyT, ValueT>::LeafNode::Slot *BTree<KeyT, ValueT>::LeafNode::lower_bound(const KeyT &key)
    {
        auto comp = [&](const Slot &slot, const KeyT &key) -> bool
        {
            assert(slot.offset > 0);
    ...
    std::optional<ValueT> BTree<KeyT, ValueT>::LeafNode::lookup(const KeyT &key)
    {
        ...
        const auto &found_key = *reinterpret_cast<KeyT *>(this->get_data() + slot->offset);
        if (found_key == key)
            return {slot->value};
