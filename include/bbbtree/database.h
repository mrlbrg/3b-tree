...
template <typename IndexT> class Database {
  ...
  private:
	...
	/// To ensure that it is destructed last. Other members might have pages to
	/// persist in their destructor.
	BufferManager buffer_manager;
};
// -----------------------------------------------------------------
template <typename IndexT>
Database<IndexT>::Database(size_t page_size, size_t num_pages, bool reset)
	: buffer_manager(page_size, num_pages),
...
// -----------------------------------------------------------------
template <typename IndexT> void Database<IndexT>::insert(Tuple &tuple) {
	// Get a new TID
...
// -----------------------------------------------------------------
template <typename IndexT> Tuple Database<IndexT>::get(Tuple::Key key) {
	// Get TID for key
	...
	// Get Tuple
	Tuple tuple{};
	auto bytes_read = records.read(
		it->second, reinterpret_cast<std::byte *>(&tuple), sizeof(tuple));
